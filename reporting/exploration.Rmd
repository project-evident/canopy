...
cl_tag = conf_all_long %>% group_by(tag) %>%
...
In the confirmation data, there are `r nrow(cl_tag)` tags and responses from `r nrow(cl_sch)` schools.
The graph below shows the distribution of the number of confirmed tags per school. 50% of schools have between `r sch_sum["1st Qu."]` and `r sch_sum["3rd Qu."]` tags, with the median school confirming `r sch_sum["Median"]` tags.
...
#     expand_limits(x = 0),
  ggplot(cl_sch, aes(x = n_tag)) +
    ...
    labs(x = "Confirmed tags per school", y = "Number of schools") +
    bar_y_scale_count +
...
ggplot(cor_df, aes(x = Correlation)) +
  ...
  geom_vline(aes(xintercept = mean(Correlation)), color = cc_cols["green"], size = 1) +
  geom_text(aes(x = mean(Correlation), y = Inf, label = paste("Average:", round(mean(Correlation), 2))), hjust = -.1, check_overlap = TRUE, vjust = 1.1, family = "Lato Light") + 
  bar_y_scale_count +
...
hc_labs = gg_hc %>%
  label %>%
  left_join(select(tags, tag), by = c("label" = "tag"))
...
ggplot() +
  ...
  geom_text(data = hc_labs, aes(x, y, label = label), hjust = 0, size = 4) +
  coord_flip(clip = "off") +
